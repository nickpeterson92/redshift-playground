...
# Post-creation script that waits for workgroup to be available
# This runs AFTER Terraform creates the workgroup resource
# It ensures the workgroup is fully ready before the next consumer starts
...
while true; do
    ...
    echo "[${NAMESPACE_NAME}] Workgroup status: $status (attempt $attempt)"
    ...
    if [ "$status" == "AVAILABLE" ]; then
        ...
        if [ -f "$LOCK_FILE/workgroup" ]; then
            ...
            if [ "$lock_workgroup" == "$WORKGROUP_NAME" ]; then
                echo "[${NAMESPACE_NAME}] Releasing our lock..."
                rm -rf "$LOCK_FILE"
            fi
        fi
        ...
        if [ "$namespace_exists" != "NOT_FOUND" ]; then
            ...
            if [ $attempt -lt 6 ]; then
                echo "[${NAMESPACE_NAME}] Workgroup not visible yet (namespace exists), AWS API propagating..."
            else
                echo "[${NAMESPACE_NAME}] Workgroup still not visible after ${attempt} attempts, waiting..."
            fi
        else
            echo "[${NAMESPACE_NAME}] Neither namespace nor workgroup found, waiting for Terraform..."
        fi
    else
        echo "[${NAMESPACE_NAME}] ⚠️ Unexpected status: $status"
    fi
    ...
    if [ $elapsed -gt $MAX_WAIT ]; then
        echo "[${NAMESPACE_NAME}] ❌ Timeout waiting for workgroup to become available"
        exit 1
    fi
...
echo "[${NAMESPACE_NAME}] Workgroup ready for next consumer to proceed"